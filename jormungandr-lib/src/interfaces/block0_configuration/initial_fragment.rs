    ...
    for message in messages {
        match message {
            ...
            Fragment::PoolRegistration(tx) => {
                let tx = tx.as_slice();
                let cert = tx.payload().into_owned();
                let auth = tx.payload_auth().into_owned();
...
fn pack_utxo_in_message(v: &[InitialUTxO]) -> Fragment {
    ...
    if outputs.len() >= 255 {
        panic!("cannot create a singular fragment with more than 254 outputs ({} requested). spread outputs to another fragment", outputs.len())
    }
