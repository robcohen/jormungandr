...
use chain_impl_mockchain::{
    ...
    fragment::Fragment,
    transaction::{
        self, InputOutput, InputOutputBuilder, Output, Payload, SetAuthData,
        SetIOs, Transaction, TransactionSignDataHash, TxBuilder, TxBuilderState,
    },
};
...
impl Staging {
    ...
    fn get_inputs_outputs(&self) -> InputOutputBuilder {
        ...
        InputOutputBuilder::new(inputs.iter(), outputs.iter())
    }
        ...
        P: Payload,
    {
        ...
        let (balance, added_outputs, _) =
            ios.seal_with_output_policy(payload, fee_algorithm, output_policy)?;
    ...
    where
        FA: FeeAlgorithm<chain::transaction::NoExtra> +
            FeeAlgorithm<chain::certificate::PoolRegistration> +
            FeeAlgorithm<chain::certificate::PoolRetirement> +
            FeeAlgorithm<chain::certificate::PoolUpdate> +
            FeeAlgorithm<chain::certificate::StakeDelegation> +
            FeeAlgorithm<chain::certificate::OwnerStakeDelegation>,
    {
        if self.kind != StagingKind::Balancing {
        ...
        match &self.extra {
            None => self.finalize_payload(&chain::transaction::NoExtra, fee_algorithm, output_policy),
    ...
    fn builder_after_witness<P: Payload>(
        ...
        Ok(builder.set_ios(&ios.inputs, &ios.outputs).set_witnesses(&witnesses))
    }

    fn make_fragment<P: Payload, F>(&self, payload: &P, auth: &P::Auth, to_fragment: F) -> Result<Fragment, Error>
    where
        F: FnOnce(Transaction<P>) -> Fragment
    {
        let tx = self.builder_after_witness(TxBuilder::new().set_payload(payload))?.set_payload_auth(auth);
        Ok(to_fragment(tx))
    }
...
mod tests {
    ...
    use chain_impl_mockchain::{key::Hash, value::Value, transaction::Input};
    use std::str::FromStr;
